              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {research.company.competitors.map((c: any, i: number) => (
                    <span key={i} className="px-3 py-1 border border-cb-border text-xs text-cb-sub font-noto">
                      {c.name}
                    </span>
                  ))}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {research.company.industry_trends.map((t: string, i: number) => (
                    <span key={i} className="px-3 py-1 bg-cb-surface text-xs text-cb-cyan font-noto">
                      {t}
                    </span>
                  ))}
                </div>
              ...
              {Object.entries(strategy.platform_strategies || {}).map(
                ([platform, data]: [string, any]) => (
                  <div key={platform} className="border border-cb-border p-4">
                    ...
                    <div className="text-[10px] text-cb-sub font-noto">優先キーワード</div>
              ...
              <div className="mt-6 pt-4 border-t border-cb-border">
                ...
                <div className="space-y-2">
                  {strategy.keyword_clusters.map((cluster: any, i: number) => (
                    <div key={i} className="flex items-center justify-between px-4 py-3 border border-cb-border">
                      <div>
                        <span className="text-sm text-cb-text font-noto">{cluster.cluster_name}</span>
                        <span className="text-xs text-cb-sub ml-3 font-noto">{cluster.pillar_keyword}</span>
                      </div>
                      <div className="flex items-center gap-3">
                        <span className="text-[10px] text-cb-sub font-noto">{cluster.content_type}</span>
                        <span className="text-[10px] text-cb-cyan font-chakra">P{cluster.priority}</span>
                      </div>
                    </div>
                  ))}
                </div>
